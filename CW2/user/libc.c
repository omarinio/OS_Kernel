...
int createPipe( pid_t send, pid_t recieve ) {
    int x;
    asm volatile("mov r0, %2 \n"
                 "mov r1, %3 \n"
                 "svc %1     \n"
              :  "=r" (x)
              :  "I" (SYS_CREATE_PIPE), "r" (send), "r" (recieve)
              :  "r0" );

    return x;
}

void writePipe( int id, uint32_t data ) {
    asm volatile("mov r0, %1 \n"
                 "mov r1, %2 \n"
                 "svc %0     \n"
              :
